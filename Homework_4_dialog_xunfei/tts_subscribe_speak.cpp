...
void xfcallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  std::string dataString = msg->data;
    ...
    {
        char nameString[40] = "我是你的小可爱。";
        text = nameString;
        std::cout<<text<<std::endl;
    }
    else if(dataString.compare("你会做什么？") == 0)
    {
        char helpString[40] = "我能陪你聊天。";
        text = helpString;
        std::cout<<text<<std::endl;
    }
    else if(dataString.compare("你在哪里住?") == 0)
    {

        char timeString[40] = "我住在你心里。";
        text = timeString;
        std::cout<<text<<std::endl;
    }
    else
  ...
  unlink("/tmp/cmd");  
  mkfifo("/tmp/cmd", 0777);  
  popen("mplayer -quiet -slave -input file=/tmp/cmd 'tts_sample1.wav'","r");
  sleep(3);
